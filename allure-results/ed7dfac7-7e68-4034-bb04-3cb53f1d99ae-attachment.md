...
- Name: Record_Cursor.spec.ts >> login and add product to cart
- Location: tests/Record_Cursor.spec.ts:3:5
...
Locator: locator('[data-test="continue"]')
Expected: visible
Timeout: 5000ms
...
```ts
  ...
  20 | 
> 21 |   await expect(page.locator('[data-test="continue"]')).toBeVisible();
     |                                                        ^ Error: expect(locator).toBeVisible() failed
  22 |   await expect(page.locator('[data-test="title"]')).toBeVisible();
  23 | 
  24 | });
```