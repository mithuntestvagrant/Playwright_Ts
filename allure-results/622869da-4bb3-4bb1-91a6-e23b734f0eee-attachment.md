...
- Name: getByRole.spec.ts >> getByRole
- Location: tests/getByRole.spec.ts:2:5
...
Call log:
  - waiting for getByRole('checkbox', { name: 'Option1' })
...
```ts
  ...
  2 | test('getByRole',async({page})=>{ 
  3 |     await page.goto('https://rahulshettyacademy.com/AutomationPractice/');
> 4 |     await page.getByRole('checkbox', { name: 'Option1' }).check();
    |                                                           ^ Error: locator.check: Test timeout of 30000ms exceeded.
  5 |     
  6 | 
  7 | });
